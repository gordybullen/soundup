  ...
  render() {
    ...
    const trackInfo = currentTrack ? (
      <div className="player-info">
        <Link id="player-artist" to="/">{artist}</Link>
        <Link id="player-title" to={`/tracks/${currentTrack}`}>
          {currentTrack.title}
        </Link>
      </div>
    ) : null;
    ...
    const playerControls = currentTrack ? (
      <div className="player-controls">
        <button className={`${icon}-btn`} onClick={this.togglePlay}>
        </button>
      </div>
    ) : null;
    const playerProgress = currentTrack ? (
      <div className="player-progress">
        ...
        <input type="range" id="progress-bar" min="0" defaultValue="0"
    ...
    return (
      <div className={currentTrack ? 'audio-player' : 'hidden'}>
